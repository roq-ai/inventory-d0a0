...
import { getStockItems, deleteStockItemById } from 'apiSdk/stock-items';
import { StockItemInterface } from 'interfaces/stock-item';

type ColumnType = ColumnDef<StockItemInterface, unknown>;

interface StockItemListPageProps {
...
export function StockItemListPage(props: StockItemListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getStockItems({
        relations: ['company', 'inventory.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<StockItemInterface>>(
    () => `/stock-items?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: StockItemInterface) => {
    if (hasAccess('stock_item', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/stock-items/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'initial_quantity', header: 'initial_quantity', accessorKey: 'initial_quantity' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/stock-items/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('stock_item', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/stock-items/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('stock_item', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="stock_item" />
        </Flex>

        {hasAccess('stock_item', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/stock-items/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
