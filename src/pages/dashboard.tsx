...
import { getStockItems } from 'apiSdk/stock-items';
import { StockItemInterface } from 'interfaces/stock-item';
import { getInventories } from 'apiSdk/inventories';
import { InventoryInterface } from 'interfaces/inventory';
import { getPurchaseOrders } from 'apiSdk/purchase-orders';
import { PurchaseOrderInterface } from 'interfaces/purchase-order';

import { SalesOrderListPage as ShowCaseList } from 'pages/sales-orders';

function DashboardPage() {
  const { session } = useSession();

  const stockItemStat = useSWR<PaginatedInterface<StockItemInterface>>(
    () => `/stock-items`,
    () => getStockItems({ limit: 1, offset: 0 }),
  );
  ...
  const purchaseOrderStat = useSWR<PaginatedInterface<PurchaseOrderInterface>>(
    () => `/purchase-orders`,
    () => getPurchaseOrders({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Stock Items',
      result: stockItemStat?.data?.totalCount,
      isLoading: stockItemStat.isLoading,
    ...
    },
    {
      ...
      result: purchaseOrderStat?.data?.totalCount,
      isLoading: purchaseOrderStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`sales-orders`} passHref legacyBehavior>
                  <Button
