...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createCompany() {
    ...
    if (body?.purchase_order?.length > 0) {
      const create_purchase_order = body.purchase_order;
      body.purchase_order = {
        create: create_purchase_order,
      };
    } else {
      delete body.purchase_order;
    }
    if (body?.sales_order?.length > 0) {
      const create_sales_order = body.sales_order;
      body.sales_order = {
        create: create_sales_order,
      };
    } else {
      delete body.sales_order;
    }
    if (body?.stock_item?.length > 0) {
      const create_stock_item = body.stock_item;
      body.stock_item = {
        create: create_stock_item,
      };
    } else {
      delete body.stock_item;
    }
