...
import { getSalesOrders, deleteSalesOrderById } from 'apiSdk/sales-orders';
import { SalesOrderInterface } from 'interfaces/sales-order';

type ColumnType = ColumnDef<SalesOrderInterface, unknown>;
...
export function SalesOrderListPage(props: SalesOrderListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getSalesOrders({
        ...
        order: params.order,
        searchTermKeys: ['order_number.contains', 'customer_name.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<SalesOrderInterface>>(
    () => `/sales-orders?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: SalesOrderInterface) => {
    if (hasAccess('sales_order', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/sales-orders/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/sales-orders/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('sales_order', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/sales-orders/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('sales_order', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="sales_order" />
        </Flex>

        {hasAccess('sales_order', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/sales-orders/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
